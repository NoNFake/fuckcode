...
describe("console account display", () => {
  ...
  test("includes the account url in account labels", () => {
    expect(Bun.stripANSI(formatAccountLabel({ email: "one@example.com", url: "https://one.example.com" }, false))).toBe(
      "one@example.com https://one.example.com",
    )
  })

  test("includes the active marker in account labels", () => {
    expect(Bun.stripANSI(formatAccountLabel({ email: "one@example.com", url: "https://one.example.com" }, true))).toBe(
