...
const TIPS: Tip[] = [
  ...
  "Create {highlight}fuckcode.json{/highlight} (or {highlight}opencode.json{/highlight}) for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/fuckcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.fuckcode/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.fuckcode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.fuckcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.fuckcode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent FuckCode from reading sensitive files",
  "Use {highlight}fuckcode run{/highlight} for non-interactive scripting",
  "Use {highlight}fuckcode --continue{/highlight} to resume the last session",
  "Use {highlight}fuckcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}fuckcode serve{/highlight} for headless API access to FuckCode",
  "Use {highlight}fuckcode run --attach{/highlight} to connect to a running server",
  "Run {highlight}fuckcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}fuckcode auth list{/highlight} to see all configured providers",
  "Run {highlight}fuckcode agent create{/highlight} for guided agent creation",
  "Use {highlight}/fuckcode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}fuckcode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/fuckcode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/fc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.fuckcode/themes/{/highlight} directory",
  ...
  "Run {highlight}fuckcode debug config{/highlight} to troubleshoot configuration",
